...
export default function Snapshots() {
  ...
  const { loading, error, data } = useQuery(SNAPSHOT_DIFF, {
    variables: { a: snapshotA, b: snapshotB },
    skip: !snapshotA || !snapshotB,
  })

  useEffect(() => {
    if (snapshotsData?.snapshots?.length >= 2) {
        ...
        <>
          ...
          <div className="diff-edges">
            {diff.addedEdges.length > 0 && (
              <>
                <h3>Added Edges</h3>
                {diff.addedEdges.map((edge: any, i: number) => (
            ...
            {diff.removedEdges.length > 0 && (
              <>
                ...
                {diff.removedEdges.map((edge: any, i: number) => (
